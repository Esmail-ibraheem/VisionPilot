...
export class BuildingsLayer {
  ...
  private material = new THREE.MeshStandardMaterial({ color: 0xe9e9e7, roughness: 0.95, metalness: 0 });
  ...
  update(mapId: string | undefined, buildings: BuildingFootprint[] | undefined): void {
    ...
    for (const b of buildings) {
      ...
      try {
        ...
        geo = new THREE.ExtrudeGeometry(shape, { depth: Math.min(b.height, 3.2), bevelEnabled: false, curveSegments: 1 });
      } catch {
        continue;
      }
