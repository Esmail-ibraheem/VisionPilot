...
- **Self-driving AI**: the population of upstream `Car`s (ray sensors + `NeuralNetwork`) trains
  ...
  upstream "save"), the next generation is mutated from it. The best car is the ego you follow; its
  sensor rays are drawn (blue clear / orange hit); the network visualizer and generation stats sit
  bottom-right; the upstream mini-map replaces the map corner. Buttons: Save brain, Discard brain,
  Next generation, show sensors / population / network.
- **Traffic**: lane-guide followers driven through the upstream car physics/controls (stop at red
  lights and stop signs, keep spacing) — the obstacles the AI's sensors see.
- **Manual mode**: drive the upstream `KEYS` car with the arrow keys (`R` respawns after a crash).
