...
![demo: driving through the virtual world](screenshots/demo.gif)

*10 s clip of the virtual world (`screenshots/demo.mp4` for the full-resolution version): the blue
ribbon traces the lane ahead, traffic queues at the lights and around the parked cars, and nothing
...
# 10 s clip (mp4 + gif); the sim is stepped 1/fps per captured frame, so it is smooth even on a
# software renderer. --setup runs JS after load, e.g. to start in manual mode driving forward.
node scripts/record.mjs --seconds 10 --fps 20 --out screenshots/demo
```
...
## Virtual world (integrated from gniziemazity/virtual-world)

The default live world is now the **virtual world** from Radu Mariescu-Istodor's
[virtual-world](https://github.com/gniziemazity/virtual-world) (MIT) — the self-driving-car course's
world editor plus its Phase-1 AI car — integrated whole, with this app's 3D visuals rendering it:

- **World editor** (`E` key or the dev panel): the upstream editor runs unchanged on its own canvas —
  draw the road graph, place stop / yield / crossing / parking / traffic-light / start / target
  markings, 🗑️ clear, 💾 save a `.world` file (also remembered in the browser), 📁 load one, 🗺️ paste
  OpenStreetMap JSON. Every edit regenerates the world and the 3D view updates immediately.
- **World generation**: roads (envelopes + polygon union), buildings, trees and lane guides are the
  upstream algorithms; this app draws them in its own style — dashed centre lines and road borders as
  perception markings, crossings as zebras, stop/yield lines with signs, signal heads with the
  upstream light controller, parking spots occupied by parked cars, the target as a ring, buildings as
  pale slabs, trees as pale low-poly trees.
- **Self-driving AI**: the population of upstream `Car`s (ray sensors + `NeuralNetwork`) trains
  generation after generation automatically — the best brain is kept (saved in the browser like the
  upstream "save"), the next generation is mutated from it. The fittest car still driving is the ego
  you follow; its displayed pose is low-pass filtered so the network's bang-bang steering does not
  jitter the view. The blue path ribbon traces the ego's lane along the road graph (straight through
  intersections); the sensor rays are hidden by default (dev panel → "sensor rays" draws them, blue
  clear / orange hit). The network visualizer and generation stats sit bottom-right; the upstream
  mini-map replaces the map corner. Buttons: Save brain, Discard brain, Next generation.
- **Traffic**: lane-guide followers driven through the upstream car physics/controls — stop at red
  lights and stop signs, keep spacing, slow for corners, pass parked cars in-lane — and the obstacles
  the AI's sensors see.
- **Solid physics**: cars are solid bodies — traffic never overlaps other traffic, the parked cars
  or your car; the AI's damage rule sees parked cars too. Buildings and trees sit behind the road
  borders, which stop every car.
- **Manual mode**: drive the upstream `KEYS` car with the arrow keys. Hitting something bumps and
  stops you (the HUD says so) — back away, or press `R` to respawn.
- Worlds: `public/worlds/default.world` (generated by `scripts/make-default-world.mjs` with the
  upstream classes) and the upstream sample town `big.world`; `?vworld=big`, or the dev panel.

Integration mechanics: `scripts/build-vw.mjs` concatenates the upstream files (from a clone in
`external/virtual-world`, `git clone https://github.com/gniziemazity/virtual-world external/virtual-world`)
into the ES module `src/vw/virtual-world.js` with three tiny patches (an accidental global `ctx`, and
DOM guards so the code also runs in tests). `src/world/vw/vwWorld.ts` runs the simulation and maps
the upstream pixel frame (50 px car = 4.7 m, y down, cars travel against a marking's direction) into
this app's `WorldState`.
...
src/
  world/              world model — independent of rendering
    ...
    simulation.ts     mode orchestration (reference / live; generated ↔ virtual world)
    vw/vwWorld.ts     virtual-world simulation adapter (population, lights, traffic → WorldState)
  vw/                 vendored virtual-world bundle (generated by scripts/build-vw.mjs) + typings
...
- Virtual world: `tests/vw.test.ts` loads the vendored bundle, generates a world, runs 20 s of the
  population + traffic (lights cycle, followers never crash, state is finite) and the generation loop.
